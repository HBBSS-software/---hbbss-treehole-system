...
function App() {
    ...
    var bgUrl = user && user.background ? user.background : '';
    return React.createElement(Router, null,
        bgUrl ? React.createElement('div', { className: 'app-background', style: { backgroundImage: 'url(' + bgUrl + ')' } }) : null,
        bgUrl ? React.createElement('div', { className: 'app-background-overlay' }) : null,
        React.createElement(Navbar, { user: user, onLogout: handleLogout, onUserUpdate: handleUserUpdate }),
        user ? React.createElement(AnnouncementPopup, { user: user }) : null,
        React.createElement(Suspense, { fallback: React.createElement('div', { className: 'loading' }, '\u52A0\u8F7D\u4E2D...') },
            React.createElement(Routes, null,
                ...
                React.createElement(Route, { path: '/favorites', element: user ? React.createElement(Favorites, { user: user }) : React.createElement(Navigate, { to: '/login' }) }),
